...
const Projects = () => {
  ...
  return (
    <section
      id="projects"
      className="py-20  lg:py-32 bg-[#0F172B] w-full border-t border-slate-800/50"
    >
      <Container className="w-full md:max-w-[720px] xl:max-w-[1140px] mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {projectsData.map((project, index) => (
            <div
              key={index}
              className="group bg-slate-900/30 border border-slate-800/60 rounded-2xl overflow-hidden shadow-xl hover:border-slate-700/60 transition-all duration-300 flex flex-col h-full"
            >
              <div className="relative w-full h-52 overflow-hidden bg-slate-950">
                ...
                />
                <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center gap-4 opacity-0 group-hover:opacity-100 transition-all duration-400">
                  <a
                    target="_blank"
                  ...
                  <a
                    ...
                    rel="noopener noreferrer"
                    href={project.git}
                    className="px-4 py-2 bg-slate-800 text-slate-200 text-sm font-semibold rounded-lg flex items-center gap-2 hover:bg-slate-700 hover:text-white transition-colors duration-300 border border-slate-700"
                  >
                    GitHub
                    <BsGithub />
                  </a>
              ...
              <div className="p-5 flex flex-col flex-grow">
                ...
                <h4 className="text-lg font-bold text-white group-hover:text-[#FE9A00] transition-colors duration-300">
                  {project.title}
                </h4>
